...
interface ProviderLoginModalProps {
    isOpen: boolean;
    onClose: () => void;
    onLoginSuccess: (provider: Provider) => void;
...
export const ProviderLoginModal: React.FC<ProviderLoginModalProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/providers/login`, {
                method: 'POST',
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
            <div className="glass-panel" style={{ width: '100%', maxWidth: '440px', padding: '2rem', background: '#ffffff', border: '1px solid #cbd5e1', position: 'relative', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
                    ...
                    <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>Provider Login</h2>
                    <p style={{ color: '#64748b', fontSize: '0.88rem', marginTop: '0.25rem' }}>
                        Log in to view customer service requests and manage your provider profile.
                    </p>
                </div>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '0 0.75rem' }}>
                            ...
                            <input
                                type="email"
                                required
                                placeholder="david.electrical@gmail.com"
                                value={email}
